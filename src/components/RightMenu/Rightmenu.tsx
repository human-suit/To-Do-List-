...
import { Ar, TextModal, Array } from "../../types/types";
...
const RightMenu: FC<Props> = ({isModal,onClick}) => {
    ...
    const [text,setText] = useState({item:{name:"",info:""}}) as TextModal | any
    ...
    const handleHange = (event:React.ChangeEvent<HTMLInputElement>) =>{
      const obj = {item:{name:event.target.value, 
        info:text.item.info}} as TextModal 
      setText(obj)
      // console.log(obj)
    }
    const handleHange2 = (event:React.ChangeEvent<HTMLInputElement>) =>{
      const obj = {item:{name:text.item.name, 
        info:event.target.value}} as TextModal 
      setText(obj)
      // console.log(obj)
    }
