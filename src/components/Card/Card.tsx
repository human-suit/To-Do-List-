...
const Card : FC<Props> = ({data}) =>{
    ...
    const handleClickPopup = (event: React.MouseEvent<HTMLButtonElement> | any) => {
        setModal(true)
    ...
    const handleClikcDelit = (event: React.MouseEvent<HTMLButtonElement> | any)=>{
        console.log(current?.value2?.initialBeans.items)
        const ar = current?.value2?.initialBeans.items! as Ar
        const num = event.target.value
        ar.splice(num-1, 1)
    ...
    return(
        ...
        <div id={String(data.beanId)} className={!data.isWork ? style.card : style.cardTrue} >
                ...
                <div className={style.but}>
                    {!data.isWork ? 
                        <button 
                            ...
                            value={String(data.beanId)}>Выполнить!
                        </button>: 
                        <button className={style.disab}
                            disabled>Выполнено:)
                        </button>
                    }
                    {!data.isWork ? 
                        <button 
                            onClick={(event)=>{handleClickPopup(event)}}
                            value={String(data.beanId)}>Редактировать
                        </button>: 
                        <button className={style.disab}
                    ...
                    <button 
                    onClick={(event)=>{handleClikcDelit(event)}}
                    value={String(data.beanId)}>Удалить</button>
                </div>
            </div>
